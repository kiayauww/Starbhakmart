$(document).ready(function () {
    ...
    function updateTotalAmount(totalAmount) {
        $('#totalAmount').text('Rp. ' + Number(totalAmount).toLocaleString('id-ID'));
    }
    ...
    function updateTotalAfterQuantityChange() {
        ...
        // Diskon hanya aktif jika total belanja mencapai minimal 100 ribu
        var discount = 0;
        if (totalPrice >= 100000) {
            discount = Math.floor(totalPrice / 100000) * 5000; // Diskon 5rb untuk setiap 100rb pembelanjaan
            totalSetelahPajak -= discount; // Kurangi diskon dari total setelah pajak
        }
    
        var totalAmount = totalSetelahPajak; // Total amount berisi nilai total setelah pajak dan diskon
        updateTotal(totalPrice);
        updatePajak(pajak);
        updateTotalAmount(totalAmount);
        updateDiscount(discount); // Perbarui tampilan diskon
    }
